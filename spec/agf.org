...
have extra work to do:
 ...
 - Sense we've just marked this page as "stuffed" (no trash within
   this page), we must update the entry's =trashlast= to point to the
   next page ID with trash in it. We can get the next page via the
   current page's =trashvor=.
