...
** Entry Type
 - =EDB_TINIT 0= - initialized page (all 0s). These should only exist
   toward the end of the file when new pages are being created. If
   there's a crash then hmmmmmmmmmmmmmmm todo
 - =EDB_TDEL   1= - =edbp_deleted=
 - =EDB_TSTRCT 2= - =edbp_struct=
 - =EDB_TOBJ   3= - =edbp_object=
 - =EDB_TENTS  4= - The index itself
 - =EDB_TPEND  5= - Temporary status between TNONE and whatever it is
   trying to become.
** Entry ID
