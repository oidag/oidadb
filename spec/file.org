...
Some Entries are reserved in fixed positions.
 ...
 - Entry 0: the edbp_index itself (=ODB_ELMENTS=)
 - Entry 1: all deleted pages (edbp_deleted, =ODB_ELMTRASH=)
   - For this entry, ref1 will point to the end of the chapter.
 - Enrty 2: edbp_struct (=ODB_ELMSTRCT=)
 - Entry 3: reserved (=ODB_ELMRSVD=)
 - Entry 4: and over: edbp_ojbect chapters (=ODB_ELMOBJ=)
...
pleft is used in non-root pages to show the left-sibling lookup page.

pright will designate the starting offset of the first
reference. Thus, a lookup page's pright will be equal to its parent's
=startoff_strait= field for the reference pointing to the page. This
is used when accessing the entity's =lastlookup= to quickly insert
object pages.
