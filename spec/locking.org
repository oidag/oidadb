...
** Object-Creating
  ...
  1. Place an SH clutch lock on the entry (thus same as [[Object-Reading]]
     step 1)
  2. Is the trash-start non-0?
     - If so: go to step XXXXXX
     - If not:
  3. With the trashlast page, before you load that page, you must
     place a XL lock on the 'trashstart_off' field in the page's header.
  ...
  8. Release the SH lock on the entry.
