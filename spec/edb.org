...
| rsvd1      | uint8_t                         |                                                                         |
| rsvd2      | uint32_t                        |                                                                         |
| rsvd       | char[72]                        | Reserved for future use. Will be all 0 bytes.                           |
...
| objectsperpage  | uint16_t | cached amount of objects per page. Or enteries if type is EDB_TENTS                          |
...
** Entry Type
 - =EDB_TINIT 0= - initialized pages (all 0s) as well as initialized
   (but unused) entries.
 - =EDB_TDEL   1= - =edbp_deleted=
 - =EDB_TSTRCT 2= - =edbp_struct=
 - =EDB_TOBJ   3= - =edbp_object=
 - =EDB_TENTS  4= - The index itself
 - =EDB_TPEND 5= - Temporary status between EDB_TINIT and whatever it
   ...
   and you see EDB_TPEND pages/entries, you can assume they are
   corrupted.
** Entry ID
