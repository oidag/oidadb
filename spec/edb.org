...
| Ref1            | [[Page ID]]  | If edbp_object: the edbp_lookup (will never be 0)                                            |
| Ref2            | [[Page ID]]  | if edbp_object: the edbp_dynamic (0 if none)                                                 |
| ref0 Count      | uint64_t | Number of pages in the chapter (used to determain offset of new pages)                       |
...
** Entry Type
 - =EDB_TINIT 0= - initialized page (all 0s). Pages of this type
   should only exist temporarly.  If you happen to start hosting and
   you see EDB_TINIT pages, you can assume they are corrupted and can
   be marked as deleted.
 - =EDB_TDEL   1= - =edbp_deleted=
 - =EDB_TSTRCT 2= - =edbp_struct=
 - =EDB_TOBJ   3= - =edbp_object=
 - =EDB_TENTS  4= - The index itself
 - =EDB_TPEND  5= - Temporary status between EDB_TINIT and whatever it is
   trying to become. Used only for traffic control. Like EDB_TINIT, any
   entries found in this type on start-up are considered corrupt.
** Entry ID
...
edbp_struct pages have consist of the array of the following
structure.
...
If we delete a structure then what do we do with that space? when can
we reuse it? Can we not utilize edbp_dynamic pages? Hmmmmmmm
...
** Structure Flags
*** State Locks
 - EDB_FSTRCT_INIT (0x80) - initialized. When the very top byte is marked as 1, that
   means this structure is live. Otherwise, this structure is assumed
   to be deleted/unitialized.
...
offset of the referenced branch, or the end page offset of the
