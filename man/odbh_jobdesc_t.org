...
* TODO - move this content to internal

Here we describe all the job types. Each job type specifies a
framework, or *protocol* to which must be followed to execute the job
when using the [[./odbh_job.org][=odbh_j*= family]].
...
* Structures
** =ODB_JSTRCTCREATE= - Create an structure
...
| Direction   | Name      | Datatype       | Notes                                                         |
|-------------+-----------+----------------+---------------------------------------------------------------|
| odbh_jwrite | odbstruct | [[./odbh_structs.org][odb_structstat]] | Structure details to be created. TODO: what to do with confv? |
| odbh_jread  | error     | odb_err        | See Errors                                                    |
| odbh_jread  | sid       | odb_sid        | The newly created structure id                                |

*** TODO Errors

** =ODB_JSTRCTDELETE= - Delete an structure
...
| Direction   | Name      | Datatype     | Notes                       |
|-------------+-----------+--------------+-----------------------------|
| odbh_jwrite | odbstruct | [[See Also][odb_entity_t]]  | See above                   |
| odbh_jread  | error     | odb_err      | See Errors                  |
| odbh_jread  | eid       | odb_eid      | The newly created entity id  |
